            ...
            <form wire:submit.prevent="submit" class="space-y-4">
                <div>
                    ...
                    <select id="level" wire:model="level" name="level" autocomplete="level" class="block mt-1 w-full rounded-md border-0 py-1.5 shadow-sm ring-1 ring-inset ring-gray-300  focus:ring-2 focus:ring-inset focus:ring-blue-800 sm:text-sm sm:leading-6 text-gray-700 bg-white border border-gray-300 rounded-lg w-80 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-500 dark:placeholder-gray-300 dark:text-gray-200 dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:ring-gray-600">
                        <option value="" disabled {{ old('level', $editEducation->level) == '' ? 'selected' : '' }}>Select Level</option>
                        <option value="Elementary" {{ old('level', $editEducation->level) == 'Elementary' ? 'selected' : '' }}>Elementary</option>
                        <option value="Secondary" {{ old('level', $editEducation->level) == 'Secondary' ? 'selected' : '' }}>Secondary</option>
                        <option value="Vocational" {{ old('level', $editEducation->level) == 'Vocational' ? 'selected' : '' }}>Vocational</option>
                        <option value="College" {{ old('level', $editEducation->level) == 'College' ? 'selected' : '' }}>College</option>
                        <option value="Graduate Studies" {{ old('level', $editEducation->level) == 'Graduate Studies' ? 'selected' : '' }}>Graduate Studies</option>
                    </select>
