<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 dark:bg-gray-900">
        ...
        <div class="w-full flex justify-end">
            ...
            wire:click.prevent="openAddModal()">
                ...
                <span class="px-1 text-xs font-semibold uppercase">Add Position</span>
            </button>
        </div>
        <!-- dropdown -->
        <div>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-200">
            ...
            @foreach($positions as $position)
                    <tr>
                        <td class="p-4">
                            <div class="flex items-center">
                            <label for="checkbox-{{ $position->id }}" class="sr-only">Select {{ $position->id }}</label>
                                <input type="checkbox" 
                        ...
                        <td class="px-6 py-4 font-semibold">{{ $position->title }}
                            <br/>
                        ...
                        <td class="px-6 py-4">{{ $position->plantilla_no ? $position->plantilla_no : 'N/A' }}</td>
                        <td class="px-6 py-4 hidden md:table-cell">{{ $position->assignment_place }}</td>
                        <td class="px-6 py-4 hidden md:table-cell">{{ $position->salary_grade }}</td>
                        <td class="px-6 py-4 hidden md:table-cell">
                            <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" id="{{ $position->id }}" class="sr-only peer" {{ $position->is_active ? 'checked' : '' }} />
                                <div wire:click.prevent="toggleActive({{ $position->id }})" 
                                    wire:confirm="This will change the status of the position. Do you want to continue?"
                        ...
                        <td class="p-2 h-32 flex items-center">
                            <div class="relative flex items-center space-x-2">
                                <a href="{{ url('positions/' . hashid_encode($position->id) ) }}" class="my-2 p-2 text-xs text-gray-400 hover:text-blue-600 dark:text-blue-300 dark:hover:text-blue-600 rounded-lg inline-flex items-center w-10 h-10" title="View Position">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m12.75 15 3-3m0 0-3-3m3 3h-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                    </svg>
                                </a>
                                <button wire:click.prevent="openEditModal({{ $position->id }})" 
                                title="Edit Entry" 
                                class="my-2 p-2 text-xs text-gray-400 hover:text-red-600 rounded inline-flex items-center w-10 h-10" title="Edit Position">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5" aria-hidden="true">
                                ...
                                class="my-2 p-2 text-xs text-gray-400 hover:text-red-600 rounded inline-flex items-center w-10 h-10">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5" title="Delete Position">
    ...
    @if (count($selected) > 0)
        <button class="mt-4 px-4 py-2 bg-red-600 text-white rounded"
            ...
            wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm removal of selected positions|DELETE">
    ...
    <div class="{{ $isOpenAdd || $isOpenEdit ? '' : 'hidden' }}">
        @if ($isOpenAdd)
            @include('hr.positions.add-modal')
        @elseif($isOpenEdit)
            @include('hr.positions.edit-modal')
        @endif
    </div>