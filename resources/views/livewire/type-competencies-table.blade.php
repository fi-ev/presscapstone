<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4">
        ...
        <div class="w-full flex justify-end">
            ...
            wire:click.prevent="openAddModal()">
                ...
                <span class="px-1 text-xs font-semibold uppercase">Add Competency Type</span>
            </button>
        </div>
        <!-- search -->
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-200">
            ...
            @foreach($competencies as $competency)
                    <tr>
                        <td class="p-4">
                            <div class="flex items-center">
                                <label for="checkbox-{{ $competency->id }}" class="sr-only">Select {{ $competency->id }}</label>
                                <input type="checkbox" 
                                    id="checkbox-{{ $competency->id }}"
                                    value="{{ $competency->id }}"
                                    wire:model.live="selected"
                                    class="w-4 h-4 text-blue-600 dark:text-blue-300 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-800 dark:border-gray-600">
                            </div>
                        </td>
                        <td class="px-6 py-4">{{ $competency->name }}</td>
                        <td class="px-2 flex justify-end">
                            <button title="Edit Entry"
                            wire:click.prevent="openEditModal({{ $competency->id }})" class="my-2 p-2 text-xs text-gray-400 hover:text-blue-600 rounded inline-flex items-center w-10 h-10">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                </svg>
                            </button>
                            <button title="Delete Entry"
                            wire:click="delete({{ $competency->id }})"
    ...
    @else
        <div class="flex items-center space-x-2 text-gray-600 text-sm">
            ...
            <span>No competencies added</span>
        </div>
    @endif
    <div class="mt-4">
        {{ $competencies->links() }}
    ...
    @if (count($selected) > 0)
        <button class="mt-4 px-4 py-2 bg-red-600 text-white rounded"
            ...
            wire:confirm.prompt="Are you sure?\n\nType DELETE to confirm removal of selected competencies|DELETE">
    ...
    <div class="{{ $isOpenAdd || $isOpenEdit ? '' : 'hidden' }}">
        @if ($isOpenAdd)
            @include('hr.competencies.add-modal')
        @elseif($isOpenEdit)
            @include('hr.competencies.edit-modal')
        @endif
    </div>