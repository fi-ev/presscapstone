<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="my-4 flex items-center justify-end gap-x-6">
        @if(!$disabled)
            <x-button class="mt-4" wire:click.prevent="openAddModal()">Add Training</x-button>
        @endif
    </div>
    <!-- table -->
    <table class="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-200">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-800 dark:text-gray-200">
            <tr>
                <th scope="col" class="px-6 py-3">
                    <b>30.</b> Title of Learning and Development Interventions / Training Programs
                </th>
                <th scope="col" class="px-6 py-3">
        ...
        @foreach($trainings as $training)
                <tr>
                    <td class="px-6 py-4">{{ $training->activity_title }}</td>
                    <td class="px-6 py-4">{{ $training->start_date->format('F j, Y') }} to {{ $training->end_date->format('F j, Y') }}</td>
                    <td class="px-6 py-4">{{ $training->hours_no }}</td>
                    <td class="px-6 py-4">{{ $training->type }}</td>
                    <td class="px-6 py-4">{{ $training->organizer }}</td>
                    <td class="p-2 h-32 flex items-center">
                        @if(!$disabled)
                        <button wire:click.prevent="openEditModal({{ $training->id }})" 
                        title="Edit Entry"
    ...
    <div class="{{ $isOpenAdd || $isOpenEdit ? '' : 'hidden' }}">
        @if ($isOpenAdd)
            @include('applicant.training.add-modal')
        @elseif($isOpenEdit)
            @include('applicant.training.edit-modal')
        @endif
    </div>