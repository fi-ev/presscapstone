<div class="relative overflow-x-auto sm:rounded-lg">
    <div class="my-4 flex items-center justify-end gap-x-6">
        @if(!$disabled)
            <x-button class="mt-4" wire:click.prevent="openAddModal()">Add Work Experience</x-button>
        @endif
    </div>
    <!-- table -->
    <table class="w-full text-sm text-left rtl:text-right text-gray-700 dark:text-gray-200">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-800 dark:text-gray-200">
            <tr>
                <th scope="col" class="px-6 py-3">
                    <b>28.</b> Inclusives Date (From & To)
                </th>
                <th scope="col" class="px-6 py-3">
        ...
        @foreach($works as $work)
                <tr>
                    <td class="px-6 py-4">{{ $work->start_date->format('F j, Y') }} to {{ $work->end_date->format('F j, Y') }}</td>
                    <td class="px-6 py-4">{{ $work->position }}</td>
                    <td class="px-6 py-4">{{ $work->company }}</td>
                    <td class="px-6 py-4">{{ number_format($work->monthly_salary,2) }}</td>
                    <td class="px-6 py-4">{{ $work->salary_grade }}</td>
                    <td class="px-6 py-4">{{ $work->appointment_status }}</td>
                    <td class="px-6 py-4">{{ $work->is_govt_service ? 'Yes' : 'No' }}</td>
                    <td class="p-2 h-32 flex items-center">
                        @if(!$disabled)
                        <button wire:click.prevent="openEditModal({{ $work->id }})" 
                        title="Edit Entry"
    ...
    <div class="{{ $isOpenAdd || $isOpenEdit ? '' : 'hidden' }}">
        @if ($isOpenAdd)
            @include('applicant.work.add-modal')
        @elseif($isOpenEdit)
            @include('applicant.work.edit-modal')
        @endif
    </div>